...
const EditorProductosExtraidos = ({ 
  ...
  useEffect(() => {
    if (productosExtraidos && productosExtraidos.length > 0) {
      console.log('🔍 EditorProductosExtraidos recibió:', productosExtraidos.length, 'productos extraídos');
      console.log('📦 Productos existentes disponibles:', productosExistentes?.length || 0);
      console.log('Productos existentes:', productosExistentes?.map(p => ({ id: p.id, nombre: p.nombre, codigo: p.codigo })));
      
      // Inicializar productos editables con sugerencias automáticas
      const productosConSugerencias = productosExtraidos.map(producto => ({
        ...producto,
        productoExistente: buscarProductoSimilar(producto, productosExistentes),
        editado: false,
        validado: false,
        errores: []
      }));
  ...
  const encontrarMejorCoincidencia = (productoExtraido, productosDB) => {
    ...
    productosDB.forEach(producto => {
      ...
      // Solo considerar como coincidencia si el score es >= 80%
      if (score > mejorScore && score >= 80) {
        mejorScore = score;
  ...
  const extraerCaracteristicas = (nombre) => {
    ...
    // Marcas conocidas
    const marcas = ['coca-cola', 'coca', 'fanta', 'sprite', 'aquarius', 'pepsi', 'mirinda', 'seven up', '7up'];
    const productos = ['cola', 'refresco', 'bebida', 'agua', 'zumo', 'jugo'];
    const sabores = ['naranja', 'limón', 'limon', 'lima', 'cereza', 'uva', 'manzana', 'piña', 'original', 'clásico', 'clasico'];
    const tipos = ['lata', 'botella', 'vidrio', 'plástico', 'plastico', 'pet'];
  ...
  const calcularSimilitud = (carac1, carac2) => {
    ...
    if (carac1.marca && carac2.marca) {
      ...
      puntosObtenidos += 15; // Puntuación parcial si ninguno tiene marca
    }
    ...
    if (carac1.sabor && carac2.sabor) {
      ...
      puntosObtenidos += 12; // Puntuación parcial
    }
    ...
    if (carac1.tipo && carac2.tipo) {
      ...
      puntosObtenidos += 7; // Puntuación parcial
    }
    
    // Coincidencia de palabras completas (peso: 10%)
    puntosTotales += 10;
    const palabrasComunes = carac1.palabrasCompletas.filter(p1 => 
      carac2.palabrasCompletas.some(p2 => p1 === p2)
    );
    const porcentajePalabras = palabrasComunes.length / Math.max(carac1.palabrasCompletas.length, carac2.palabrasCompletas.length);
    puntosObtenidos += Math.round(porcentajePalabras * 10);
    
    return Math.round((puntosObtenidos / puntosTotales) * 100);
  };
  ...
  const filtrarProductos = (busqueda) => {
    ...
    .filter(producto => producto.score > 30) // Solo mostrar productos con score > 30%
          ...
          <div className="flex">
            ...
            <div className="text-sm text-blue-800">
              ...
              <ul className="list-disc list-inside space-y-1">
                <li>Revisa cada producto extraído y corrige cualquier error</li>
                <li>Asigna productos existentes cuando sea posible para mantener la consistencia</li>
                <li>Los precios deben ser de <strong>compra al proveedor</strong>, no de venta</li>
                <li>Valida cada producto antes de guardar</li>
              </ul>
            </div>
          </div>
        </div>
                  ...
                  {producto.productoExistente ? (
                    <div className="bg-green-100 border border-green-200 rounded p-3">
                        ...
                        <div className="flex items-center">
                          ...
                          {producto.productoExistente.scoreAsignacion && (
                            <span className={`ml-2 text-xs px-2 py-1 rounded ${
                              producto.productoExistente.scoreAsignacion >= 90 ? 'bg-green-200 text-green-800' :
                              producto.productoExistente.scoreAsignacion >= 80 ? 'bg-yellow-200 text-yellow-800' :
                      ...
                      <div className="text-sm text-green-800">
                        ...
                        {producto.productoExistente.scoreAsignacion && producto.productoExistente.scoreAsignacion < 90 && (
                          <p className="text-xs text-yellow-700 mt-1 font-medium">
                            ⚠️ Revisa esta asignación - confianza media
                          </p>
                        )}
                      </div>
                    </div>
                                ...
                                <div className="flex justify-between items-start">
                                  ...
                                  <div className="ml-2">
                                    <span className={`text-xs px-2 py-1 rounded ${
                                      producto_db.score >= 80 ? 'bg-green-100 text-green-700' :
                                      producto_db.score >= 60 ? 'bg-yellow-100 text-yellow-700' :
